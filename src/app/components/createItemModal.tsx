...
export default function CreateItemModal({
  ...
  const [open, setOpen] = useState<boolean>(false); // State to manage modal open/close

  // Function to handle form submission for creating an item
    ...
    <Dialog.Root open={open} onOpenChange={setOpen}>
      <Dialog.Trigger asChild>
        <button
          ...
          onClick={() => setOpen(true)} // Open the modal when button is clicked
        >
          Create Item
        </button>
      </Dialog.Trigger>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed bg-white p-6 rounded-md shadow-md top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-[90%] max-w-md">
          <Dialog.Title className="text-lg font-bold">Create New Item</Dialog.Title>
